(() => {
    ...
    browser.runtime.onInstalled.addListener(async (details) => {
        if (
            ...
            const newKeys = {
                [automaticKey]: keys[automaticKey] ?? (keys[requestKey] || keys[updateKey]) ?? true,
                [newPagesOnlyKey]: keys[requestKey] === false || false,
            };
    ...
    browser.webRequest.onBeforeRequest.addListener(
        ...
        async (details) => {
            ...
            if (details.url && url?.[0]) {
                const { [automaticKey]: auto } = await browser.storage.sync.get([automaticKey]);
